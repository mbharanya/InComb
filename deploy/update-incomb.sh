...
#Dump db
echo "dumping db..."
mysqldump -u$MYSQL_USER -p$MYSQL_PW incomb content news > $BACKUPDIR/content.sql
echo "done!"
...
echo "loading backed up sql..."
cat $BACKUPDIR/content.sql | mysql -u$MYSQL_USER -p$MYSQL_PW $MYSQL_TABLE
echo "done!"
